...
<% HttpSession hs=request.getSession(); 
	...
	String names="";
	int price=0;
	for(HashMap<String,String> p: al)
	{
		price+=Integer.parseInt(p.get("price"))*Integer.parseInt(p.get("qty"));
		names=names+p.get("pname")+",";
	}
%>
...
<input type="hidden" name="key" value="<%= merchant_key %>" />
      ...
      <table>
        ...
        <tr>
          <td>Amount: </td>
          <td><input name="amount" value="<%=price %>" /></td>
          <td>First Name: </td>
          <td><input name="firstname" id="firstname" value="<%= ob.getN() %>" /></td>
        </tr>
        <tr>
          <td>Email: </td>
          <td><input name="email" id="email" value="<%= ob.getE()%>" /></td>
          <td>Phone: </td>
          <td><input name="phone" value="<%= ob.getPh() %>" /></td>
        </tr>
        <tr>
          <td>Product Info: </td>
          <td colspan="3"><input name="productinfo" value="<%=names %>" size="64" /></td>
        </tr>
        <tr>
          <td>Success URI: </td>
          <td colspan="3"><input name="surl" value="http://localhost:8080/Shoppingcart/Success" size="64" /></td>
        </tr>
        <tr>
          <td>Failure URI: </td>
          <td colspan="3"><input name="furl" value="http://localhost:8080/Shoppingcart/Failure.jsp" size="64" /></td>
        </tr>
        <tr>
